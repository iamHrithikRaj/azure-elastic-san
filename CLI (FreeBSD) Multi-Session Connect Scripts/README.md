...
```
# BEGIN AZURE ELASTIC SAN MANAGED BLOCK -- DO NOT EDIT (generated by ...)
...stanzas...
# END AZURE ELASTIC SAN MANAGED BLOCK
```
...
already has a managed block from a previous run, it is fully replaced (not
appended to); if it has only one of the `BEGIN`/`END` markers (hand-edited or
corrupted), the script refuses to touch the file and tells you to fix it
